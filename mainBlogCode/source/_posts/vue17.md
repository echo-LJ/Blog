...
**`$children/$parent通信`**
在Vue中，可以使用`$children`和`$parent`来实现非结构化的组件通信，即在`嵌套的组件`之间进行数据传递和方法调用，同时不需要进行显式的声明和注册。
...
⚠️：当子组件中有多个父组件的时候（子组件为动态组件），`$parent`获取到的结果是子组件的第一个父组件。
...
需要注意的是，`$attrs`和`$listeners`需要和其他属性和事件配合使用，具体使用时还需结合具体场景来选择是否需要使用。
