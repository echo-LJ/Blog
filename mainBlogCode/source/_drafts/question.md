...
多人在线协同操作，实时同步数据（websocket）

底层： spreadJs 实现。 — 目前全球最强在线表格厂商，飞书、腾讯底层也是spreadJS实现。
...
快捷键操作（复制/粘贴）+ 工具栏操作（筛选/字体字号，插入链接，下拉菜单） ——> 调用spreadJS的命令 ——> spreadJS执行完成，吐出异步回调事件 ——-> 我们监听事件，发送websocket给服务端 ———> 服务端处理数据，存储入库，websocket同步给协作者 ——> 协作者页面事件还原
...
客户端：生产消息时，会记录messageId === 时间戳
服务端：消息到达时间， 记录msgUid === 先到先得 ——》 记录发生乱序埋点 —-〉短信/邮件

TCP：全双工，可信（确定顺序）。 网络层 ===》 应用层；
webSocket 保持长链接原理： 心跳： 10s一个 

[1,2,3] ===〉 websocket.send()

历史记录。每一个OP生成一个记录，每10个OP形成一个版本或特定时间断内生成一个版本

数据准确性：
 所有人的页面展示都一致。 所有人的数据都来自于历史版本，历史版本的形成基于websocket长链接先到先得规则

载入页面/页面刷新
 拉取服务端数据，历史版本： （1）最近的一个版本；（2）最近一个版本之后的记录；



（2）大OP存储优化
正常OP / 压缩OP> 4w：pako / 大OP: length > 10w
大OP上传oss，（1）ws会断，数据丢失；（2）减轻服务端存储压力；

过程： 上传OSS，获得ossUrl，封装到OP中，同步给服务端。

发生错误：三次重试，降级方案


（3）离线消息补偿
原理：indexDB ——》 dexie插件 （add新增记录、put置换某一条记录、update更新某一记录、delete删除记录、IDB.each()循环取记录执行）

优化方案： 内存存储:刷新/重载即丢失 ===》 本地存储 （cookie：4k、localStorage：5M、indexDB：理论上无限大，存几百M没问题）

过程： 
新OP产生 —》 增加indexDB记录 —〉 收到ws消息回执，删除记录
刷新/重载/ws断链后重连成功 遍历数据库执行消息补偿。

调研了竞品 1h、3h、1d、3d 都是直接覆盖，不会删数据


（4）性能问题
spreadJS，canvas-GUI，
消耗时间地方： spreadJS执行cmd、调接口发同步消息
